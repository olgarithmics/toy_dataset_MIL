...
def center_point(mask):
	...
	for cordinates in prop:
		...
		center_coordinates.append((int(temp_center[0]), int(temp_center[1])))
	...
	return np.clip(center_mask, a_min=0, a_max=1).astype(np.uint8), center_coordinates
...
def center_edge(mask, image):
	center_map, center_coordinates=center_point(mask)
	...
	return check_image.astype(np.uint8), comb_mask.astype(np.uint8),center_coordinates